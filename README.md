...
3. 등교 시각 / 목표 취침 시각 (앱 알람은 기본 꺼짐 — 쓰려면 '앱 알람' 칸에 시각 입력)
...
| 항목 | 출처 | 갱신 |
|---|---|---|
| 날씨·미세먼지·자외선·일출입 | 기상청(키 입력 시) / Open-Meteo | 15분 |
| 삼성 라이온즈 경기 | 네이버 스포츠 (비공식) | 6시간 |
| 뉴스 3건 | 구글 뉴스 토픽 RSS (실패 시 검색 RSS) | 3시간 |
| 운세 | 앱 내부 명리 계산 (통신 없음) | 매일 |

샤오미 워치 수면 점수는 외부에서 읽어올 방법이 없어서, 카드의 **Mi 피트니스 열기** 버튼으로 앱을 열고 숫자만 옮겨 적는 방식이다. 적기 전까지는 빈 상태로 둔다.
...
- `날씨-정확하게.md` — 기상청 키를 넣어 네이버와 같은 예보 쓰기
